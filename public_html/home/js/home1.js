    ...
    $(document).ready(function () {
        ...
        console.log(min)
            ...
            for (i=0 ;i<=(diff)/5000; i++ ){
                  content +=`
            <option value="${Number(max)-(i*5000)}">${addCommas(Number(max)-(i*5000))}  </option>
            `
            }
            $('#s2').html(content)
        ...
        $('body').on('change','#video-cover',function (e) {
            var filename = $(this).val().split('\\').pop();
            console.log(filename)

            $("#cname").text(filename)
            noty('فایل  انتخاب شد ','green','')
     ...
     var filename =element.val().split('\\').pop();
        ...
        $('input[type=radio][name=pricech]').change(function() {
            var element=$(this)
            var data=element.data('mo')
            console .log(element.val())
            $('#amount').val(0)
            var dd=addCommas(data)
            $('.amount').text(dd+' تومان ')
        });
        ...
        $(document).on('submit','#port_form',function(){
            noty('در حال بارگذاری اطلاعات ...','green','پیغام')
            $('body').LoadingOverlay("show"  )
        });
        ...
        $('input[type=radio][name=type]').change(function() {
            var val=$(this).val()
            if (val =='education'){
                $('#onv').text('عنوان سابقه کار را بنویسید')
            }
            if (val =='licence'){
                $('#onv').text('عنوان   مدارک را بنویسید')
            }
            if (val =='sabeghe'){
                $('#onv').text('عنوان   تحصیلات  را بنویسید')
            }
        ...
        $('body').on('click', '#more_lang', function(event){
            $('#more_lang_sec').fadeToggle(400)
        })
   ...
   $('.remove').click(function() {
       setTimeout(function() {
        ...
        $('#teacher-clander .con ul li .hour.open').mouseenter(function(){
            ...
            var totop= $(this).offset().top-parent.offset().top-260;
    ...
    //    console.log('class_id'+class_id)
            ...
            var d='<div class="resbox" style="top:'+totop+'px; margin-left:'+ml+'px; left:'+toleft+'px;">'+
                ...
                '<li><span> شروع :</span><span class="vla">'+ftime+ '</span><span class="vla">-</span><span class="vla">به مدت یک ساعت</span></li>'+
                '</ul>'+
                '</div>'+
                '<div class="but">'+
        ...
        $("input[type='radio'][name='class_type']").click(function() {
            ...
            switch (count){
                ...
                case 5:
                case 10:
                    $('.class_time').text('60 دقیقه') ; break;
            }
            switch (count){
                case 0:
                    $('.class_time').text('30 دقیقه') ;
